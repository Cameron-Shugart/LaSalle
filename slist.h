...
public:
	...
	void add(void* value); //Adds a new value to the end of this list.
	...
	void insert(int index, void* value);		//Inserts the element into this list before the specified index.
	...
	//void mapAll(void (*f)(Airport))	;			//Calls the specified function on each element of the linkedlist in ascending index order.

	void remove(int index);		//Removes the element at the specified index from this list.

	void set(int index, void* value);		//Replaces the element at the specified index in this list with a new value.
